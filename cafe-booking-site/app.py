...
def generate_reviews(c, cafe_id):
    # Генерируем от 5 до 15 отзывов на кафе
    count = random.randint(5, 15)
    for _ in range(count):
        name = random.choice(R_NAMES)
        rating = random.choices([5, 4, 3, 2], weights=[50, 30, 15, 5])[0]  # Чаще хорошие
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        ...
        for idx, cafe in enumerate(cafes_data):
            ...
            # Генерируем отзывы
            generate_reviews(c, cafe_id)

            # Генерация столов (та же логика, сокращенно для экономии места, но функционал тот же)
            tables = []
            if cafe_id == 1:  # Уют
                for r in range(3):
                    for col in range(4):
                        tables.append((f"{r + 1}-{col + 1}", 4 if r == 1 else 2, 10 + col * 25, 15 + r * 30, "rect", 0))
            elif cafe_id == 2:  # Вкусняшка (Г)
            ...
            elif cafe_id == 3:  # Sky (Круг)
                for i in range(8):
                    angle = (i / 8) * 2 * math.pi;
                    x = 50 + 35 * math.cos(angle) - 5;
                    y = 50 + 35 * math.sin(angle) - 5
                    tables.append((f"R{i + 1}", 2, int(x), int(y), "round", 0))
                tables.extend(
                    [("V1", 6, 5, 5, "square", 45), ("V2", 6, 85, 5, "square", -45), ("V3", 6, 5, 85, "square", -45),
                     ("V4", 6, 85, 85, "square", 45)])
            elif cafe_id == 4:  # Токио
...
def booking():
    ...
    # Получаем отзывы для этого кафе
...
# Login/Register/Profile/Logout остаются прежними, но Profile чуть обновим ниже
...
def profile():
    ...
    conn = get_db_connection()
    # Получаем бонусы
    user = conn.execute("SELECT bonus_points FROM users WHERE id=?", (session['user_id'],)).fetchone()
    bookings = conn.execute("""
        SELECT b.date, b.time, c.name as cafe_name, t.number as table_number, t.seats
